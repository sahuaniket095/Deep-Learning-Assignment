...
try:
    ...
    logging.error(f"joblib is not installed: {e}")
    raise HTTPException(status_code=500, detail="joblib is not installed")
...
app = FastAPI(
    ...
    description="A FastAPI server to provide predictions.",
    version="1.0.0"
)
...
@app.post("/predict/{model_name}", summary="Get prediction from a model")
...
def home():
    ...
    """
    return {"message": "Welcome to the FastAPI Prediction Server"}
